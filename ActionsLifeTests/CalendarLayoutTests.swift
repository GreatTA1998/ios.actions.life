...
final class CalendarLayoutTests: XCTestCase {
    ...
    func testHourScrollHitTestFindsPaintedCardNotBlockFrame() {
        // `0884a7e` title tap was empty-hour create because `blockFrame`
        // missed the painted card. `UIScrollView.hitTest` must see the
        // card UIView at hour 7; hour 4 (empty) must not.
        let hourH: CGFloat = 50
        ...
        let card = UIView(frame: CGRect(x: 6, y: 7 * hourH, width: 208, height: 36))
        card.accessibilityIdentifier = CalendarLayout.timedCardAccessibilityID("pr3")
        canvas.addSubview(card)

        let body = UIView(frame: CGRect(x: 0, y: 0, width: 208, height: 20))
        ...
        scroll.contentOffset = CGPoint(x: 0, y: 4 * hourH)
        scroll.layoutIfNeeded()
        ...
        XCTAssertEqual(
            hit(CGPoint(x: 100, y: 7 * hourH + 8)),
            .blockBody(taskID: "pr3"),
            "hour-7 title must be Details, not empty-hour create"
        )
        XCTAssertEqual(
            hit(CGPoint(x: 100, y: 7 * hourH + 28)),
            .capsule(taskID: "pr3"),
            "bottom 16pt must be setDuration"
        )
        XCTAssertNil(
            hit(CGPoint(x: 100, y: 4 * hourH + 10)),
            "empty hour 4 must stay timed create, not a stolen card hit"
        )
    }

    func testPaintedCardHitUsesLocalYWhenOnlyCardIDExists() {
        let hourH: CGFloat = 50
        let scroll = UIScrollView(frame: CGRect(x: 0, y: 0, width: 220, height: 400))
        let canvas = UIView(frame: CGRect(x: 0, y: 0, width: 220, height: 24 * hourH))
        scroll.addSubview(canvas)
        ...
        XCTAssertEqual(hit(CGPoint(x: 100, y: 7 * hourH + 8)), .blockBody(taskID: "pr3"))
    ...
    func testEmptyHourHitNeverUsesBlockFrameForDetails() {
        ...
        // A title-like Y would be blockBody in hourCanvasHit. After hitTest
        // misses, emptyHourHit must still create — not Details-from-frame.
        let titleY = placed[0].y + 8
        XCTAssertEqual(
            CalendarLayout.emptyHourHit(
                contentPoint: CGPoint(x: 40, y: titleY),
                columns: columns,
                columnWidth: 220,
                pixelsPerHour: 50,
                snap: 15
            ...
            .emptyHour(
                dayISO: "2026-09-19",
                minutes: CalendarLayout.minutes(atY: titleY, pixelsPerHour: 50, snap: 15)
            )
        )
        XCTAssertEqual(
            CalendarLayout.emptyHourHit(
            ...
            .emptyHour(
                dayISO: "2026-09-19",
                minutes: CalendarLayout.minutes(atY: 4 * 50 + 10, pixelsPerHour: 50, snap: 15)
            )
        )
    }
    ...
    func testBlockFrameIsOnlyPaintedPixelsNotSpacerHost() {
        ...
        let frame = CalendarLayout.blockFrame(event: placed, columnWidth: 220)
        XCTAssertEqual(frame.minY, 7 * 50, accuracy: 0.01)
        XCTAssertEqual(frame.height, 36, accuracy: 0.01)
        XCTAssertFalse(frame.contains(CGPoint(x: 40, y: 4 * 50 + 10)))
        XCTAssertTrue(frame.contains(CGPoint(x: 40, y: 7 * 50 + 8)))
        let capsule = CalendarLayout.durationCapsuleRect(
            columnIndex: 0,
            columnWidth: 220,
            event: placed
        )
        XCTAssertTrue(capsule.contains(CGPoint(x: capsule.midX, y: capsule.midY)))
        XCTAssertFalse(capsule.contains(CGPoint(x: 40, y: 7 * 50 + 8)))
    }
